...
Deconvolution::Deconvolution(const std::shared_ptr<ngraph::Node>& op,
    ...
    if (isSupportedOperation(op, errorMessage)) {
        ...
        if (convBackprop) {
            ...
            deconvAttrs.biasesDims = {deconvAttrs.OC};
            ...
            deconvAttrs.biasesDims = {deconvAttrs.OC * deconvAttrs.groupNum};
        ...
        for (int i = 0; i < deconvAttrs.dilation.size(); i++) {
...
InferenceEngine::Blob::Ptr Deconvolution::createWeiBlobAsIO(InferenceEngine::SizeVector dims) {
    ...
    for (int i = 2 + deconvAttrs.withGroups; i < dimsForBlockedDesc.size(); i++)
...
bool Deconvolution::canBeExecutedInInt8() const {
    ...
    for (int i = 0; i < deconvAttrs.kernel.size(); i++) {
...
std::pair<VectorDims, VectorDims> Deconvolution::makeDummyInOutShape() {
    ...
    if (isDynamicNode()) {
        ...
        if (deconvAttrs.externOutShape) {
            ...
            ov::CoordinateDiff pb = deconvAttrs.autoPad ? ov::CoordinateDiff(deconvAttrs.paddingL.size(), 0) : deconvAttrs.paddingL;
            ov::CoordinateDiff pe = deconvAttrs.autoPad ? ov::CoordinateDiff(deconvAttrs.paddingR.size(), 0) : deconvAttrs.paddingR;
            ...
            for (size_t i = 0; i < inputDims.size() - 2; i++) {
                if (origInDims[2 + i] == Shape::UNDEFINED_DIM) {
                    inputDims[2 + i] = ((deconvAttrs.lastOutputSpatialDims[i] - (deconvAttrs.dilation[i] + 1) *
                                        (weightDims[wghOffset + 2 + i] - 1) - 1 + pb[i] + pe[i] - deconvAttrs.outputPadding[i])) /
...
void Deconvolution::initPaddingR(const Shape &inShape, const Shape &outShape) {
    for (int i = 0; i < deconvAttrs.paddingR.size(); i++) {
...
void Deconvolution::createPrimitive() {
    if (deconvAttrs.isInt8) {
        ...
        if (biasDesc != nullptr)
            ...
            dnnlBiasDesc = biasDesc->getDnnlDesc().reshape({DnnlExtensionUtils::convertToDnnlDim(deconvAttrs.biasesDims[0])});
        auto desc = createInt8MkldnnDeconvDesc(inDesc->getDnnlDesc(), wgh_candidate, dnnlBiasDesc, outDesc->getDnnlDesc(), deconvAttrs.withBiases,
                                               deconvAttrs.stride, deconvAttrs.dilation, deconvAttrs.paddingL, deconvAttrs.paddingR);
...
void Deconvolution::createDescriptor(const std::vector<MemoryDescPtr> &inputDesc,
    ...
    if (deconvAttrs.isInt8) {
        if (deconvAttrs.withBiases) {
            ...
            bias_candidate = dnnl::memory::desc(DnnlExtensionUtils::convertToDnnlDims(deconvAttrs.biasesDims), bdt, memory::format_tag::any);
        }
        ...
        descs.emplace_back(createDescriptorInternalInt8(in_candidate, wgh_candidate, bias_candidate,
                                                        ...
                                                        deconvAttrs.dilation, deconvAttrs.paddingL, deconvAttrs.paddingR));
    } else {
        ...
        for (auto alg : {dnnl::algorithm::convolution_winograd, dnnl::algorithm::convolution_direct}) {
            ...
            std::tie(deconv_desc, fwd_conv_pd) = createDescriptorInternalDefault(in_candidate, wgh_candidate, out_candidate, alg,
                                                                                 ...
                                                                                 deconvAttrs.paddingR, getEngine());
            if (fwd_conv_pd->get(true) == nullptr)
                continue;
...
std::shared_ptr<MemoryDesc> Deconvolution::getSrcMemDesc(dnnl::primitive_desc_iterator &primitive_desc_it, size_t idx) {
    ...
    auto desc = idx > 0 ? primitive_desc_it.weights_desc(idx - 1) : deconvAttrs.isInt8 ? primitive_desc_it.src_desc(idx) : primitive_desc_it.diff_dst_desc(idx);
...
std::shared_ptr<MemoryDesc> Deconvolution::getDstMemDesc(dnnl::primitive_desc_iterator &primitive_desc_it, size_t idx) {
    auto desc =  deconvAttrs.isInt8 ? primitive_desc_it.dst_desc(idx) : primitive_desc_it.diff_src_desc(idx);
