...
bool AclDeconvExecutor::init(const DeconvAttrs& deconvAttrs,
    ...
    unsigned int pad_l = deconvAttrs.paddingL.at(1) < 0 ? 0 : deconvAttrs.paddingL.at(1);
    unsigned int pad_r = deconvAttrs.paddingR.at(1) < 0 ? 0 : deconvAttrs.paddingR.at(1);
    unsigned int pad_t = deconvAttrs.paddingL.at(0) < 0 ? 0 : deconvAttrs.paddingL.at(0);
    ...
    std::cout << "AclDeconvExecutor::init pad_l: " << pad_l << std::endl;
    ...
    if (!status) {
        ...
        std::cout << "NEDeconvolutionLayer validation failed: " << status.error_description() << std::endl;
        return false;
    } else {
        std::cout << "NEDeconvolutionLayer validation passed" << std::endl;
    }
