...
JitReduceExecutor::JitReduceExecutor() : ReduceExecutor() {}
...
bool JitReduceExecutor::Key::operator==(const ReduceKey &rhs) const {
...
bool JitReduceExecutor::init(const ReduceAttrs& reduceAttrs,
    ...
    jcp.src_dt = srcDescs[0]->getPrecision();
    jcp.dst_dt = dstDescs[0]->getPrecision();
    ...
    if (srcDescs[0]->hasLayoutType(LayoutType::ncsp)) {
        jcp.layout = ReduceLayoutType::reduce_ncsp;
    } else if (srcDescs[0]->hasLayoutType(LayoutType::nspc)) {
        jcp.layout = ReduceLayoutType::reduce_nspc;
    } else {
        jcp.layout = ReduceLayoutType::reduce_blocked;
    }
...
void JitReduceExecutor::exec(const std::vector<MemoryCPtr>& src, 
...
void JitReduceExecutor::reduce_type(const uint8_t *in_ptr, uint8_t *out_ptr, size_t dst_size) {
...
void JitReduceExecutor::reduce_PLN(const uint8_t *in_ptr, uint8_t *out_ptr) {
...
void JitReduceExecutor::reduce_BLK(const uint8_t *in_ptr, uint8_t *out_ptr) {
...
void JitReduceExecutor::reduce_BLK_concern_padding(const uint8_t *in_ptr, uint8_t *out_ptr) {
    size_t ICB = div_up(IC, blk_size);
    size_t OCB = div_up(OC, blk_size);
...
inline void JitReduceExecutor::reduce_kernel_post_process(uint8_t *out_ptr) {
