...
void Transformations::PreLpt(const std::vector<ov::element::Type>& defaultPrecisions, const bool isLegacyApi) {
    ...
    CPU_REGISTER_PASS_ARM64(manager, ConvertReduceMultiAxis);
    ...
    CPU_ENABLE_PASS_COMMON(manager, ngraph::pass::SoftmaxDecomposition);

    CPU_SET_CALLBACK_COMMON(manager,
            [](const_node_ptr &node) -> bool {
                return node->input_value(0).get_partial_shape().rank().get_length() <= 5;
            },
        ngraph::pass::SoftmaxDecomposition);
...
void Transformations::MainSnippets(void) {
    ...
    if (snippetsMode != Config::SnippetsMode::IgnoreCallback)
        CPU_REGISTER_PASS_X64(snippetsManager, SnippetsMarkSkipped);
